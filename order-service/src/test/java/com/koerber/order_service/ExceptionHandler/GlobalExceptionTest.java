...
public class GlobalExceptionTest {
    ...
    void handleInsufficientInventoryException() {
        InsufficientInventoryException ex = mock(InsufficientInventoryException.class);
        when(ex.getMessage()).thenReturn("No static resource h2-console for request '/h2-console'.");

        ResponseEntity<ApiError> response =globalException.handleInsufficientInventory(ex);
    ...
    @Test
    void handleInventoryServiceException() {
        InventoryServiceException ex = mock(InventoryServiceException.class);
        when(ex.getMessage()).thenReturn("No static resource h2-console for request '/h2-console'.");

        ResponseEntity<ApiError> response =globalException.handleInventoryService(ex);
